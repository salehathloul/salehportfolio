...
export default function ShareButtons({ url, title, locale }: ShareButtonsProps) {
  ...
  return (
    <div className="sb-wrap" aria-label={label}>
      {/* X / Twitter */}
      <a
        href={xHref}
        target="_blank"
        ...
        className="sb-btn"
        aria-label={locale === "ar" ? "مشاركة على X" : "Share on X"}
      ...
      <a
        ...
        rel="noopener noreferrer"
        className="sb-btn"
        aria-label={locale === "ar" ? "مشاركة عبر واتساب" : "Share on WhatsApp"}
        title={locale === "ar" ? "مشاركة عبر واتساب" : "Share on WhatsApp"}
      >
      ...
      <button
        onClick={handleCopy}
        className={`sb-btn sb-copy${copied ? " sb-copy--done" : ""}`}
        aria-label={
          copied
